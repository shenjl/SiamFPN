...
class SiamFPN(FPN):
    ...
    def track(self, detection):
        ...
        for i in range(len(detection_features)):
            ...
            pred_score = F.conv2d(conv_scores, self.cls_kernels[i], groups=N).reshape(N, 2 * self.anchor_num,
                                                                                      score_size, score_size)

            conv_reg = conv_regression.reshape(1, -1, d_shape, d_shape)
            pred_regression = self.regress_adjust(
                F.conv2d(conv_reg, self.reg_kernels[i], groups=N).reshape(N, 4 * self.anchor_num, score_size,
                                                                          score_size))
    ...
    def forward(self, template, detection):
        ...
        for i in range(len(template_features)):
            ...
            pred_score = F.conv2d(conv_scores, score_filters, groups=N).reshape(N, 2 * self.anchor_num,
                                                                                feature_map_size, feature_map_size)

            conv_reg = conv_regression.reshape(1, -1, d_shape, d_shape)
            reg_filters = kernel_regression.reshape(-1, 256, t_shape, t_shape)
            pred_regression = self.regress_adjust(
                F.conv2d(conv_reg, reg_filters, groups=N).reshape(N, 4 * self.anchor_num, feature_map_size,
                                                                  feature_map_size))
...
if __name__ == "__main__":
    ...
    pred_scores, pred_regressions = net.train(Variable(torch.randn(2, 3, 127, 127)),
                                              Variable(torch.randn(2, 3, 271, 271)))
